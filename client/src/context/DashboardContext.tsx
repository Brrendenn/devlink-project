...
export const DashboardProvider = ({ children }: { children: ReactNode }) => {
    ...
    const [profile, setProfile] = useState<Profile>({ imageUrl: '', theme: 'light'});
    ...
    if (token) {
      const fetchInitialData = async () => {
        ...
        try {
          ...
          const linksWithClientIds = userLinks.map((link: any) => ({ ...link, id: `client-${Math.random()}` }));
  ...
  return (
    <DashboardContext.Provider value={{ links, profile, setLinks, setProfile, isLoading }}>
        {children}
